...
#include "Windows/LoginWindow.hpp"
#include "Windows/MainWindow.hpp"
#include "Fonts/RudaBold.hpp"
#include "Textures/InfinityLogo.hpp"
...
int main( ) {
	auto vanguard = LoadLibraryA( "vanguard.dll" );

	if( !vanguard ) {
		Utils::Shout( "Failed to load vanguard.dll", g_title.c_str( ), true );
	}

	Login = ( t_Login )GetProcAddress( vanguard, "Login" );
	Logout = ( t_Logout )GetProcAddress( vanguard, "Logout" );
	GetStatus = ( t_Status )GetProcAddress( vanguard, "GetStatus" );
	GetUsername = ( t_Username )GetProcAddress( vanguard, "GetUsername" );
	GetModules = ( t_Modules )GetProcAddress( vanguard, "GetModuleList" );
	Inject = ( t_Inject )GetProcAddress( vanguard, "Inject" );
...
bool CreateDeviceD3D( ) {
	...
	D3DXCreateTextureFromFileInMemoryEx( g_pd3dDevice, &InfinityLogo, sizeof( InfinityLogo ), 200, 120, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &g_pLogoTexture );
...
LRESULT WINAPI WndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam ) {
	...
	case WM_SYSCOMMAND:
		if( ( wParam & 0xfff0 ) == SC_KEYMENU ) // no alt key pls
			return 0;
		break;

		/*case WM_NCHITTEST: {
			LRESULT hit = DefWindowProc(hWnd, msg, wParam, lParam); // drag window by client area
